...
class App {
  ...
  _getPosition() {
    if (navigator.geolocation) {
      //getCurrentPosition(success callback, failure callback)
      navigator.geolocation.getCurrentPosition(this._loadMap.bind(this), () =>
        alert("Cant access your location. Please try again.")
      );
    }
  }

  _loadMap(position) {
    //destructing to get latitude and longitude.
    const { latitude } = position.coords;
    const { longitude } = position.coords;

    ///Using leaflet library for hosting map
  ...
  _newWorkout(e) {
    ...
    if (type === "cycling") {
      ...
      if (
        ...
        return alert("Input has to be a valid number.");
  ...
  _moveToPopup(e) {
    ...
    this.#map.setView(workoutData.coords, 17, {
      animate: true,
      pan: {
        duration: 1,
      },
